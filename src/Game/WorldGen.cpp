...
namespace BinF::Game {
   ...
   using Hash = u32;
    ...
    static inline Hash Hash2D(s32 x, s32 y) {
        Hash h = WorldSeed;
    ...
    static inline f32 ValueNoise2D(BinF::f32 x, f32 y) {
        ...
        const f32 n00 = NormaliseHash(Hash2D(x0, y0));
        const f32 n10 = NormaliseHash(Hash2D(x1, y0));
        const f32 n01 = NormaliseHash(Hash2D(x0, y1));
        const f32 n11 = NormaliseHash(Hash2D(x1, y1));
